...
class NoteTaker {
  ...
  static generateNotes() {
    ...
    return [Note.newNote('Разходете се сред природата. Ще Ви се отрази добре.', undefined), Note.newNote('Пари', undefined),
      ...
      Note.newNote('Ще си имаш бебче - пухкаво и бяло като хлебче!', undefined),
      ...
      Note.newNote('Любов и щастие ще пълнят твойта гръд, и рози ще застилят твоя път.', undefined),
      ...
      Note.newNote('Змията винаги твърди, че хапе с лечебна цел.<br/>| The snake always claims that bites with curative intent. |', undefined),
      ...
      Note.newNote('През какво да преминем избира съдбата, но как да преминем избираме ние.<br/>| The destiny elects what to pass through, but we choose how to pass. |', undefined),
      ...
      Note.newNote('Кое е по-важно за успеха - талантът или труда? А кое колело на велосипеда е по-важно - предното или задното?.' +
        '<br/>| What is more important for success - the talent or the labor? And which wheel of the bicycle is more important - the front or the rear one? |', undefined),
      Note.newNote('Можем ли да освободим света от злосторници? Вероятно толкова, колкото и да го населим със Светци.' +
        '<br/>| Can we rid the world of evildoers? Probably as much as to populate it with saints. |', 'Тома Томов | Toma Tomov'),
      ...
      Note.newNote('Ей го слънцето! Утрото е хубаво, колко по-хубав ще бъде денят!', undefined),
      Note.newNote('Колкото повече се потиш по време на мир, толкова по-малко ще кървиш по време на война.', 'Норман Шварцкопф'),
      Note.newNote('Животът е една изпита чаща, а водата все тече... тече... тече.', undefined),
      Note.newNote('Както бистрата вода оглежда слънцето от синьото небе, тъй душата е с надежда да проблесне в чистото сърце.', undefined),
      Note.newNote('Ромоли чешмата - пей, напява<br/>за далечни стари времена<br/>в спомени мили образи представя<br/>на лица със светли имена.', undefined),
      Note.newNote('Бързо и безспир текат водите<br/>бързо и животът ни тече<br/>и затуй, о брате мой към висините,<br/>ум безспир насочвай и сърце.', undefined),
      Note.newNote('Музиката в душата може да бъде чута от Вселената.<br/>Бъдете доволни от това, което имате. Радвайте се на нещата такива каквито са.<br/>Когато осъзнаете, че нищо не ви липсва, целият свят ви принадлежи.<br/>Ако осъзнаете, че всичко се променя, няма да се крепите за нищо.<br/>И най-дългото пътешествие започва с първата крачка.', 'Лао Дзъ'),
      Note.newNote('Онова, което не можеш да избегнеш, посрещни с добре дошло.', 'Китайска мъдрост'),
      Note.newNote('Кой е най-щастливия момент от деня? - Рано сутрин с роса по тревата.', 'Боян Петров'),
      Note.newNote('Някои от най-хубавите ти дни, все още не са се случили.', undefined),
      Note.newNote('Всеки ден излизай на разходка. Чудесата стават навсякъде.', undefined),
      Note.newNote('Една блага дума често е по-освежаваща от водата и сянката.', 'Буда'),
